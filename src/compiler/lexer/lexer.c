...
int set_token(Token *t, TokenDef type, const char *value) {
  ...
  default:
    strlcpy(t->value.stringValue, value, sizeof(t->value.stringValue));
    break;
  }
  return 1;
}

static void putback(int c) { cs.putback = c; }
...
static int match_sequence(int c, const char *seq, TokenDef tokenType,
                          Token *t) {
  const char *ptr = seq;
...
int match_identifier(Token *t, char c) {
  ...
  reportError("Lexer error: Unrecognised character ", &c, cs.line, cs.column);
  return 0;
}
...
int scan(Token *t) {
  ...
  switch (c) {
  case 'R':
    return match_sequence(c, "Room", T_ROOM, t);
    break;
  case 'M':
    return match_sequence(c, "Map", T_MAP, t);
    break;
  case 'C':
    return match_sequence(c, "Connect", T_CONNECT, t);
    break;
  case '-':
    return match_sequence(c, "->", T_DIRECTED_EDGE, t);
    break;
  case '<':
    return match_sequence(c, "<->", T_BIDIRECTIONAL_EDGE, t);
    break;

  case ';':
    return set_token(t, T_SEMICOLON, ";");
    break;
  case '{':
    return set_token(t, T_LBRACKET, "{");
    break;
  case '}':
    return set_token(t, T_RBRACKET, "}");
    break;
  case '(':
    return set_token(t, T_LPAREN, "(");
    break;
  case ')':
    return set_token(t, T_RPAREN, ")");
    break;
  case EOF:
    return 0;
  default:
